...
import { shareVisitToGoogleMaps } from '../../services/leaderboard';
...
type Props = {
  ...
  placeDocId: string;
  placeName: string;
  /** Google Place ID — enables Share to Google Maps review */
  placeId?: string | null;
  /** When `history`, opens on past visits; `log` shows the new-visit form first */
...
export default function VisitLogDialog({
  ...
  const handleShareToGoogleMaps = async (visit: PlaceVisit) => {
    if (!placeId) {
      setError('This temple has no Google Place ID — open it from Google Maps search first, or save with a placeId.');
      return;
    }
    setSharingVisitId(String(visit._id));
    setShareHint(null);
    setError(null);
    try {
      const { copied } = await shareVisitToGoogleMaps({
        placeId,
      ...
      setShareHint(
        copied
          ? 'Remarks copied. Paste them into the Google Maps review. Photos/videos must be attached in Maps manually.'
          : 'Opened Google Maps review. Paste your remarks and attach photos there — Google does not allow apps to post reviews directly.'
      );
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Could not open Google Maps');
    } finally {
  ...
  const pastVisitsSection = (
    <div className={activeView === 'log' ? `border-t pt-4 mt-2 ${isDark ? 'border-white/10' : 'border-[#e5e5e5]'}` : ''}>
      {activeView === 'log' && (
              ...
              >
                ...
                {(v.media?.length ?? 0) > 0 && (
                  <div
                    className={`grid gap-2 ${
                      activeView === 'history' ? 'grid-cols-2 sm:grid-cols-3' : 'grid-cols-3'
                    }`}
                  >
                    {v.media.map((m) => (
                      <div
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {placeId && (
                    <button
                      type="button"
                      disabled={sharingVisitId === id}
                      onClick={() => void handleShareToGoogleMaps(v)}
                      className={`text-xs flex items-center gap-1 font-medium ${
                        isDark ? 'text-[#2DD4BF] hover:underline' : 'text-[#0D9488] hover:underline'
                      } disabled:opacity-50`}
                    >
                      {sharingVisitId === id ? (
                        <Loader2 className="w-3 h-3 animate-spin" />
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription className={muted}>
              {activeView === 'history' ? (
                <>
                  Your visit history for <span className="font-medium">{placeName}</span>.
                </>
              ) : (
                <>
                ...
                {shareHint && (
                  <p
                    className={`text-xs rounded-lg px-3 py-2 mb-3 ${
                      isDark ? 'bg-[#0D9488]/15 text-[#2DD4BF]' : 'bg-[#0D9488]/10 text-[#0D9488]'
                    }`}
                  >
                    {shareHint}
                  </p>
