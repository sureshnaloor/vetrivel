...
export default function DivyaDesamsExplorer() {
  ...
  const { lists, myLists, adoptedParentIds, loading, cloneList } = useDivyaDesam();
    ...
    <DashboardShell>
      <header className="mb-8">
        <h1 className="font-display text-4xl font-semibold mb-2">
          Explore Divya Desams & Curated Lists
        </h1>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {sourceLists.map((list) => {
            ...
            const listId = normalizeDocumentId(list._id);
            const isAdopted = listId != null && adoptedParentIds.has(listId);
            return (
              <div
                key={list._id}
              ...
              >
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    {isTemplate && (
                      <span
                        className={`inline-block text-[10px] uppercase tracking-wider px-2 py-1 rounded font-semibold ${
                          isDark ? 'bg-[#0D9488]/20 text-[#2DD4BF]' : 'bg-[#0D9488]/15 text-[#0D9488]'
                        }`}
                      >
                        Global Template
                      </span>
                    ...
                    {isAdopted && (
                      <span
                        className={`inline-flex items-center gap-1 text-[10px] uppercase tracking-wider px-2 py-1 rounded font-semibold ${
                ...
                <div className="mt-6 pt-4 border-t border-dashed border-gray-500/20">
                  {isAdopted ? (
                    <button
                      onClick={() => {
                        const adoptedList = myLists.find(
                          (l) => normalizeDocumentId(l.parentListId) === listId
                        );
                      ...
                      className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-xl font-medium transition-colors ${
                        isDark
                          ...
                          : 'bg-black/5 text-[#141414] hover:bg-black/10'
                      }`}
                    >
